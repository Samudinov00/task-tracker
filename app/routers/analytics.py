...
from app.models.project import Notification, Project, Task, PRIORITY_CHOICES, STATUS_CHOICES, STATUS_PRODUCTION
...
def _get_export_tasks(db: Session, user: User):
    return (
        ...
        .order_by(Project.name, Task.status, Task.title)
        .all()
    )
...
async def analytics(request: Request, db: Session = Depends(get_db)):
    ...
    all_tasks = tasks_q.options(joinedload(Task.assignee)).all()

    status_labels = [label for _, label in STATUS_CHOICES]
    status_data = [sum(1 for t in all_tasks if t.status == key) for key, _ in STATUS_CHOICES]
    ...
    production_count = sum(1 for t in all_tasks if t.status == STATUS_PRODUCTION)
    overdue_count = sum(1 for t in all_tasks if t.deadline and t.deadline < today and t.status != STATUS_PRODUCTION)
...
async def export_tasks_csv(request: Request, db: Session = Depends(get_db)):
    ...
    for task in tasks:
        ...
        total_min = sum(tl.minutes for tl in task.time_logs)
        writer.writerow([
            task.title,
            task.project.name,
...
async def export_tasks_excel(request: Request, db: Session = Depends(get_db)):
    ...
    for row_idx, task in enumerate(tasks, 2):
        ...
        ws.cell(row=row_idx, column=8, value=total_min)
